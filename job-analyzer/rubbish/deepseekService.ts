...
/**
 * DeepSeek API服务（通过DMXAPI聚合平台）
 */

/**
 * 调用DeepSeek API进行分析（通过DMXAPI）
 * @param request 分析请求参数
 * @returns 分析结果
 */
export async function analyzeWithDeepSeek(request: AnalyzeRequest): Promise<AnalysisResult> {
  ...
  try {
    ...
    // 解析响应
    return parseAnalysisResponse(responseText, 'deepseek');
  } catch (error) {
    // 处理错误
