...
/**
 * OpenAI API服务（通过DMXAPI聚合平台）
 */

/**
 * 调用OpenAI API进行分析（通过DMXAPI）
 * @param request 分析请求参数
 * @returns 分析结果
 */
export async function analyzeWithOpenAI(request: AnalyzeRequest): Promise<AnalysisResult> {
  ...
  try {
    ...
    // 初始化OpenAI客户端（使用DMXAPI的base_url）
    ...
    // 解析响应
    return parseAnalysisResponse(responseText, 'openai');
  } catch (error) {
    // 处理错误
